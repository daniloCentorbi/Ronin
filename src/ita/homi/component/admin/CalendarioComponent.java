...
public class CalendarioComponent extends VerticalLayout {
	...
	public CalendarioComponent(
			int numero, int codicecliente) {
		...
		*/
		calendarComponent.setHandler(new BasicDateClickHandler() {
			...
			public void dateClick(DateClickEvent event) {
				...
				switchToDayView();
				//showEventPopup(CalendarTestEvent,  true);
			}
		});
...
/*
	...
	private void showEventPopup(CalendarTestEvent event, boolean newEvent) {
		...
		try {
			...
			System.out.println("NO SELECTEDEVENT");
			e.printStackTrace();
		}
	...
	*/
	...
	private void commitCalendarEvent() {
		...
		if (!dataSource.containsEvent(event)) {
			...
			event.setFlag("settato");
			event.setCodiceordine(23);
	...
	*/
	...
	public void addInitialEvents(String composition) {
		...
		if (composition.equals("all")) {
			...
			try {
				elenco = bd.getEventFromUser(codicecliente);
			} catch (GestoreException e) {
